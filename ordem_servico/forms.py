...
class OrdemServicoForm(forms.Form):
    ...
    descricao = forms.CharField(max_length=51, label="Descrição:", help_text="Descrever a Descrição (51 Caracteres)", widget=forms.Textarea(attrs={'rows': 3, 'cols': 40}))
...
class OrdemServicoArquivadoForm(forms.Form):
    ...
    def clean_log(self, username):
        ...
        solicitacao = self.cleaned_data['solicitacao']
        executado = self.cleaned_data['executado']
        dados = f"Data: {data} | Serviço: {servico} | Quantidade: {quantidade} | Tempo de Execução: {execucao} | Valor: {valor} | Autorização: {autorizacao} | Solicitação {solicitacaoLocal} - {solicitacao} | Executado por: {executado}" 
        request_project_log(cod, dados, "ORDEM DE SERVIÇO / Create Ordem Arquivada", username)